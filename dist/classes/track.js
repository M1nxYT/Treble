    ...
    static async from(url, interaction, client) {
        ...
        let methods = {
            async onStart() {
                ...
                if (!subscription) {
                    try {
                        return await interaction.followUp({ content: `An error occurred, Try queue that song again!`, ephemeral: true }).catch(console.warn);
                    }
                    catch (err) {
                        console.log(err);
                    }
                }
                if (!subscription.audioPlayer) {
                    try {
                        return await interaction.followUp({ content: `An error occurred, Try queue that song again!`, ephemeral: true }).catch(console.warn);
                    }
                    catch (err) {
                ...
                let imageForColor = song.thumbnails.filter((thumbnail) => thumbnail.url.includes('.jpg'));
                try {
                    const nowPlayingEmbed = new discord_js_1.MessageEmbed()
            ...
            async onFinish() {
                ...
                if (!subscription) {
                    try {
                        return await interaction.followUp({ content: `An error occurred`, ephemeral: true }).catch(console.warn);
                    }
                    catch (err) {
                        console.log(err);
                    }
                ...
                if (subscription.queue.length == 0 && interaction.guildId && subscription.voiceConnection) {
                    ...
                    try {
                        await interaction.followUp({ content: `Left channel, no songs left in queue!`, ephemeral: true });
                    }
                    catch (err) {
                        console.log(err);
                    }
                    ...
                    try {
                        await interaction.followUp({ content: 'Now finished!', ephemeral: true }).catch(console.warn);
                    }
                    catch (err) {
                        console.log(err);
                    }
            ...
            async onError(error) {
                console.warn(error);
                try {
                    await interaction.followUp({ content: `Error: ${error.message}`, ephemeral: true }).catch(console.warn);
                }
                catch (err) {
                    console.log(err);
                }
