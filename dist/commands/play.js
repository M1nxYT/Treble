...
exports.play = {
    ...
    data: new SlashCommandBuilder().setName('play').setDescription('Play a Song!').addStringOption((option) => option.setName('song').setDescription('Url or Song Name').setRequired(true)),
    ephemeral: true,
    async execute(interaction, client) {
        ...
        if (url.includes('radio')) { // if youtube radio link
            ...
            for (let video in videos) {
                try {
                    const track = await track_1.Track.from(`https://www.youtube.com/watch?v=${list.videos[video].videoId}`, interaction, client);
                ...
                if (album.tracks) {
                    ...
                    for (let song in songs) {
                        ...
                        if (spotifyInfo.artists.length != 1) {
                            ...
                            artists = spotifyInfo.artists[0].name;
                        }
                        const url = await (await yts(`${spotifyInfo.name} - ${artists} `)).videos[0].url;
                        const track = await track_1.Track.from(url, interaction, client);
                        subscription.enqueue(track);
            ...
            const response = new MessageActionRow().addComponents(new MessageSelectMenu()
                .setCustomId('search')
